...
/*   Updated: 2026/01/27 15:42:24 by lantonio         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
std::vector<std::string> Server::_input_builder(Client* _client, char *newBuffer, int bytesRead) {
	std::vector<std::string> comandos;

	(void)bytesRead;
	_client->setRecvBuffer(_client->getRecvBuffer() + newBuffer);
	//bufferAcumulado.append(newBuffer, bytesRead);

	size_t pos;
	while ((pos = _client->getRecvBuffer().find("\r\n")) != std::string::npos)
	{
		std::string cmd = _client->getRecvBuffer().substr(0, pos);
